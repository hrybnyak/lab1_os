...
size_t allign_to_four(size_t size) {
...
void alloc::Allocator::mem_free(void *ptr) {
  ...
  auto *prev_chunk = get_prev(chunk);
  auto *next_chunk = get_next(chunk);
  if (next_chunk->free) merge(chunk, next_chunk);
  if (prev_chunk->free) merge(prev_chunk, chunk);
}
